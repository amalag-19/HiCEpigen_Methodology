...
set.seed(1)
randomly_sampled_row_IDs<-sample(x = 1:nrow(df_chr_IDs_dist),size = nrow(df_chr_IDs_dist)/2,replace = F) 
...
#save(lm_fit,file = "/Users/Amal/Box Sync/PSU/Fall 2018/BioStat_Research/Apps/HiCEpigen/lm_fit_obj_model1.RData")
...
matrix_output<-matrix_generator_lm(start_ID=501, end_ID= 1000, chr_mat = chr_mat_full,TAD_locations = TAD_locations,lm_obj = lm_fit)
...
par(mfrow=c(1,1))
plot(x = matrix_output[[4]],y = matrix_output[[5]])
matrix_output[[6]]
