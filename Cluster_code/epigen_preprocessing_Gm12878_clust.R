...
df_epigen<-fread(input = "/storage/home/a/aua257/work/Bio_Stat/penalized_full_models/data/GM12878_ENCODE36.bigBed",data.table = F)
...
# ## saving the states
# save(states,file="/Users/Amal/Box Sync/PSU/Fall 2018/BioStat_Research/Data/Epigenetic/states_Gm12878.RData")
...
save(df_epigen_output,file="/storage/home/a/aua257/work/Bio_Stat/penalized_full_models/results/df_epigen_output_Gm12878.RData")

# state_j_counter<-function(df,window_start_ID,window_end_ID,chromStart_current_indicator=TRUE){
#   count<-0
#   if(chromStart_current_indicator){
#     for(index in 1:nrow(df)){
#       if(df$chromEnd[index]<=window_end_ID){
#         count<-count+floor((df$chromEnd[index]-df$chromStart[index]+1)/200)
#       }else{
#         count<-count+floor((window_end_ID-df$chromStart[index]+1)/200)
#       }
#     }
#   }else{
#     for(index in 1:nrow(df)){
#       if(df$chromEnd[index]<=window_end_ID){
#         count<-count+floor((df$chromEnd[index]-window_start_ID+1)/200)
#       }else{
#         count<-count+floor((window_end_ID-window_start_ID+1)/200)
#       }
#     }
#   }
#   return(count)
...
#   window_end_IDs[i]<-i*10000
#   
#   df_epigen_chr1_subset<-df_epigen_chr1[which(max(df_epigen_chr1$chromStart,window_start_IDs[i])<=min(df_epigen_chr1$chromEnd,window_end_IDs[i])),]
#   
#   
#   chromStart_current_window<-((df_epigen_chr1$chromStart>=window_start_IDs[i])&(df_epigen_chr1$chromStart<=window_end_IDs[i]))
#   if(i==1){
#     window_i_chromStart_rows<-which(chromStart_current_window)
#     if(length(window_i_chromStart_rows)>=1){
#       df_epigen_chr1_window_i_chromStart<-df_epigen_chr1[window_i_chromStart_rows,]
#       for(j in 1:length(states)){
#         state_j_row_IDs<-which(df_epigen_chr1_window_i_chromStart$name==states[j])
#         if(length(state_j_row_IDs)>=1){
#           df_epigen_chr1_window_i_chromStart_state_j<-df_epigen_chr1_window_i_chromStart[,]
#           state_j_count<-state_j_counter(df = df_epigen_chr1_window_i_chromStart_state_j, window_start_ID = window_start_IDs[i], window_end_ID = window_end_IDs[i], chromStart_current_indicator=TRUE)
#         }
#        
#       }
#     }
...
#     
#     
#     chromStart_previous_window<-((df_epigen_chr1$chromStart>=window_start_IDs[i-1])&(df_epigen_chr1$chromStart<=window_end_IDs[i-1]))
#     chromEnd_current_or_next_window<-(df_epigen_chr1$chromEnd>=window_start_IDs[i])
#     
#     window_i_chromStart_rows<-which(chromStart_current_window)
#     
#     
#     
#     window_i_rows<-which(chromStart_previous_window&chromEnd_current_or_next_window)
#     
#   }
#   
#   df_epigen_chr1[window_i_rows,]
# }
