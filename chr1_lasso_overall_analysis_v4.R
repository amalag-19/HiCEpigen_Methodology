#########################################################################################################
## Building linear model with constraints on first 500 window with TAD locations as predictors
#########################################################################################################
...
lm_fit<-penalized(response = log(Intensity+1), penalized = as.matrix(df_chr[,-1]) , lambda1=0, lambda2=0, positive = rep(T,ncol(df_chr)-1), data=df_chr)
...
test_stat<-l1-l2
p_val<-2*(1-pchisq(q = test_stat,df = 70))
p_val
